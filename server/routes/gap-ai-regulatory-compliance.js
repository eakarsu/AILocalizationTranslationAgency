...
// Route: /api/gap-ai-regulatory-compliance
// Feature: Ai Regulatory Compliance
// Description: (per-locale legal terminology check)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Regulatory Compliance. (per-locale legal terminology check)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Regulatory Compliance\nDescription: (per-locale legal terminology check)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-regulatory-compliance', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-regulatory-compliance', title: 'Ai Regulatory Compliance', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-regulatory-compliance', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-regulatory-compliance']);
      return res.json({ slug: 'ai-regulatory-compliance', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-regulatory-compliance', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-regulatory-compliance', items: [], error: (e && e.message) || 'Server error' });
  }
});
