...
// Route: /api/gap-multi-pass
// Feature: Multi Pass
// Description: Multi-pass QA workflow (reviewer, auditor approvals)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Multi Pass. Multi-pass QA workflow (reviewer, auditor approvals)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'multi-pass', title: 'Multi Pass', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'multi-pass', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['multi-pass']);
      return res.json({ slug: 'multi-pass', items: r.rows || [] });
    }
    return res.json({ slug: 'multi-pass', items: [] });
  } catch (e) {
    return res.json({ slug: 'multi-pass', items: [], error: (e && e.message) || 'Server error' });
  }
});
