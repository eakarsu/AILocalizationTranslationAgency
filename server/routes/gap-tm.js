...
// Route: /api/gap-tm
// Feature: Tm
// Description: TM synchronization & leverage metrics dashboard
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Tm. TM synchronization & leverage metrics dashboard' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'tm', title: 'Tm', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'tm', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['tm']);
      return res.json({ slug: 'tm', items: r.rows || [] });
    }
    return res.json({ slug: 'tm', items: [] });
  } catch (e) {
    return res.json({ slug: 'tm', items: [], error: (e && e.message) || 'Server error' });
  }
});
