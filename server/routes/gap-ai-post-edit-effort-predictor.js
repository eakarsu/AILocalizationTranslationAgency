...
// Route: /api/gap-ai-post-edit-effort-predictor
// Feature: Ai Post Edit Effort Predictor
// Description: (estimate post-edit cost)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Post Edit Effort Predictor. (estimate post-edit cost)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Post Edit Effort Predictor\nDescription: (estimate post-edit cost)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-post-edit-effort-predictor', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-post-edit-effort-predictor', title: 'Ai Post Edit Effort Predictor', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-post-edit-effort-predictor', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-post-edit-effort-predictor']);
      return res.json({ slug: 'ai-post-edit-effort-predictor', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-post-edit-effort-predictor', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-post-edit-effort-predictor', items: [], error: (e && e.message) || 'Server error' });
  }
});
