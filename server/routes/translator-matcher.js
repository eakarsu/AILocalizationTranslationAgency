// Auto-generated feature route: translator-matcher
// Domain: translation operations matchmaker — Capability: assigning translators by expertise + language pair + subject matter
// Mounted under: /api/translator-matcher
...
// POST /api/translator-matcher/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a translation operations matchmaker. Your task is assigning translators by expertise + language pair + subject matter. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/translator-matcher/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/translator-matcher/:id — fetch one
...
// DELETE /api/translator-matcher/:id — discard a run
