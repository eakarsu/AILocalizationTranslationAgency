...
// Route: /api/gap-in-context
// Feature: In Context
// Description: In-context side-by-side review tool
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: In Context. In-context side-by-side review tool' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'in-context', title: 'In Context', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'in-context', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['in-context']);
      return res.json({ slug: 'in-context', items: r.rows || [] });
    }
    return res.json({ slug: 'in-context', items: [] });
  } catch (e) {
    return res.json({ slug: 'in-context', items: [], error: (e && e.message) || 'Server error' });
  }
});
