...
// Route: /api/gap-ai-dialect-variant-detection
// Feature: Ai Dialect Variant Detection
// Description: /ai/dialect-variant-detection
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Dialect Variant Detection. /ai/dialect-variant-detection' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Dialect Variant Detection\nDescription: /ai/dialect-variant-detection\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-dialect-variant-detection', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-dialect-variant-detection', title: 'Ai Dialect Variant Detection', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-dialect-variant-detection', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-dialect-variant-detection']);
      return res.json({ slug: 'ai-dialect-variant-detection', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-dialect-variant-detection', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-dialect-variant-detection', items: [], error: (e && e.message) || 'Server error' });
  }
});
