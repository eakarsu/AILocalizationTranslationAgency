// Auto-generated feature route: qa-stream
// Domain: translation QA reviewer — Capability: streaming QA against glossary and tone
// Mounted under: /api/qa-stream
...
// POST /api/qa-stream/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a translation QA reviewer. Your task is streaming QA against glossary and tone. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/qa-stream/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/qa-stream/:id — fetch one
...
// DELETE /api/qa-stream/:id — discard a run
