...
// Route: /api/gap-ai-realtime-tm-leverage
// Feature: Ai Realtime Tm Leverage
// Description: (live TM hit rate during translation)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Realtime Tm Leverage. (live TM hit rate during translation)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Realtime Tm Leverage\nDescription: (live TM hit rate during translation)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-realtime-tm-leverage', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-realtime-tm-leverage', title: 'Ai Realtime Tm Leverage', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-realtime-tm-leverage', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-realtime-tm-leverage']);
      return res.json({ slug: 'ai-realtime-tm-leverage', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-realtime-tm-leverage', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-realtime-tm-leverage', items: [], error: (e && e.message) || 'Server error' });
  }
});
