...
const ENDPOINT = '/interp/streaming-asr';
const TITLE = 'Streaming ASR';
...
const FIELDS = [
  ...
  { name: 'domain_model', label: 'Domain Model', type: 'text' },
  { name: 'status', label: 'Status', type: 'select', options: ['active', 'paused', 'completed', 'error'] },
  { name: 'noise_level', label: 'Noise Level', type: 'text' },
  { name: 'confidence_score', label: 'Confidence Score', type: 'number' },
  { name: 'speaker_accent', label: 'Speaker Accent', type: 'text' },
  { name: 'partial_transcript', label: 'Partial Transcript', type: 'textarea' },
  { name: 'final_transcript', label: 'Final Transcript', type: 'textarea' },
];

const AI_VERBS = [
  'classify-audio-quality',
  'suggest-language-detected',
  'predict-recognition-accuracy',
  'detect-noise-source',
  'recommend-noise-reduction',
  'generate-vad-segments',
  'summarize-asr-session',
  'score-asr-confidence',
  'validate-audio-format',
  'suggest-sample-rate-change',
  'detect-overlapping-speech',
  'recommend-domain-model',
  'classify-speaker-accent',
  'predict-rebuffer-need',
  'generate-asr-corrections',
  'suggest-restart-stream',
];
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{ maxWidth: 600 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-actions" style={{ justifyContent: 'flex-start' }}>
          <button className="btn btn-primary" style={{ width: 'auto' }} onClick={runVerb} disabled={loading}>
            {loading ? 'Running...' : 'Run Verb'}
          </button>
        </div>
        ...
        {result && (
          <div style={{ marginTop: 12, padding: 12, background: '#f0fdf4', borderRadius: 6 }}>
            <strong>Verb:</strong> {result.verb}<br />
            <strong>Generated At:</strong> {result.generatedAt}<br />
...
export default function InterpStreamingAsr() {
              ...
              <tbody>
                {items.map(item => (
                  <tr key={item.id} onClick={() => setSelected(item)} style={{ cursor: 'pointer' }}>
                    ...
                    {displayCols.map(f => (
                      <td key={f.name}>
                        {f.name === 'status' ? <span className={`status-badge status-${item[f.name]}`}>{item[f.name]}</span> : (item[f.name] != null ? String(item[f.name]) : '-')}
                      </td>
                    ))}
            ...
            <div className="detail-grid">
              ...
              {FIELDS.map(f => (
                <div key={f.name} className={`detail-item ${f.type === 'textarea' ? 'full-width' : ''}`}>
                  ...
                  <div className="detail-value">{selected[f.name] != null ? String(selected[f.name]) : '-'}</div>
                </div>
              ))}
